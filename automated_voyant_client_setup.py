...
def run_command(command, capture=False):  # Runs a shell command and returns the output
    result = subprocess.run(
        command, env=env, shell=True, capture_output=capture, text=True
    )
...
def get_lidar_interface_name():  # Function to get the interface name for lidar with user input
    ...
    for interface_name in interface_names:
        if any(ext in interface_name for ext in ethernet_extensions):
            logging.info(
                f"   {interface_names.index(interface_name) + 1}. {interface_name} \033[32m<- likely lidar\033[0m"
            )
        else:
            logging.info(
                f"   {interface_names.index(interface_name) + 1}. {interface_name}"
            )

    selected_interface = input(
        "Please enter number to select network interface of LiDAR: "
    )

    if (
        selected_interface.isdigit()
        and int(selected_interface) > 0
        and int(selected_interface) <= len(interface_names)
    ):
...
if len(network_interfaces) == 0:
    logging.error(
        "No network interfaces found. Make sure your lidar is connected via ethernet to this device and powered on. Once done, rerun this script. Exiting setup."
    )
    exit(1)
...
# Verify connection
logging.info(
    f"Network lidar interface {selected_interface} configured with IP address. Testing connection."
)
if not is_device_reachable("192.168.20.20"):
    logging.error(
        "Lidar device is unresponsive. Please check the connection and ensure the device is powered on, then rerun this script. Exiting setup."
    )
    exit(1)
else:
...
logging.info(
    "Voyant Client setup completed successfully. You can now use the Voyant SDK inside the Docker container. For more information, refer to the documentation at https://voyant-photonics.github.io/."
)
